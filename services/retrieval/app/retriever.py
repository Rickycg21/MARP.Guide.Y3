...
class Retriever:
    ...
    Thin wrapper around:
      ...
      - Query embedding (HTTP endpoint or deterministic dev fallback)
    ...
    def __init__(
        ...
        self.collection = collection or os.getenv("CHROMA_COLLECTION") or "marp_chunks"
    ...
    async def health(self) -> Dict[str, Any]:
        ...
        embed_ok = True if not self.embed_ep else await self._ping_embedder()
    ...
    async def _ping_embedder(self) -> bool:
        """Small POST to verify the embedding endpoint responds."""
        try:
    ...
    async def search(
        ...
        """
        Embed the query, run vector search in Chroma, and return normalized rows.
        ...
        for i, doc in enumerate(docs):
            ...
            if sim is not None:
                sim = max(0.0, min(1.0, sim))  # clamp to [0,1] for UI
            rows.append({
        ...
        # Keep stats minimal; framework timing will cover latency in logs/APM
    ...
    async def _embed(self, text: str) -> List[float]:
        """
        Try external embedder, otherwise use deterministic 3D fallback that
        aligns with our seed vectors for local testing.
        """
        if self.embed_ep:
            try:
                async with httpx.AsyncClient(timeout=8.0) as c:
                    r = await c.post(self.embed_ep, json={"input": text})
                    r.raise_for_status()
                    return r.json()["data"][0]["embedding"]
            except Exception as e:
                log.warning("embed endpoint failed, using fallback: %s", e)

        # Dev fallback
        t = (text or "").lower()
        return [0.1, 0.2, 0.7] if any(k in t for k in ("envelope","event","correlation","timestamp")) else [0.9,0.1,0.0]
